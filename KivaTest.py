...
# Get all of the recent lenders, searching for 'UT' and 'Utah'
# Utah
...
# UT
newLendersRange = simplejson.loads(urllib.urlopen('http://api.kivaws.org/v1/lenders/search.json?q=UT&sort_by=newest').read())['paging']['pages']
i = 1
while i <= newLendersRange:
    urlStr = 'http://api.kivaws.org/v1/lenders/search.json?q=UT&country_code=US&sort_by=newest&page=' + str(i) # Load each of the pages
    newLenders = simplejson.loads(urllib.urlopen(urlStr).read())['lenders']
    i = i+1
    for j in newLenders:
        lenderID = j['uid']
